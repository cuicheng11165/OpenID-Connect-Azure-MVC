...
对于开启了multi-tenant的OpenId认证的app，需要domain管理员**以管理员身份**consent后，enduser才可以登录。


consent 地址：https://login.microsoftonline.com/{tenantId}/adminconsent?client_id={clientId}&redirect_uri={一个合法的reply url}。 **必须是这个地址，这里demo中默认弹出的consent页面是以enduser身份去consent的**
